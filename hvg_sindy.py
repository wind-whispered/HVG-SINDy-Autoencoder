...
def fit_exponential_distribution(degree_dist):
    """
    Fit exponential decay of the degree distribution: P(k) ~ exp(-λk)
    λ is the chaos criterion:
    - λ ≈ ln(3/2) ≈ 0.405 indicates random noise
    - λ < ln(3/2) may indicate chaos
    - λ > ln(3/2) may indicate periodicity
    ...
    # Filter zero values for log fitting
    mask = p_values > 0
    k_fit = k_values[mask]
...
#     """
#     Plot degree distribution with exponential fit
#
#     Parameters:
#     -----------
#     degree_dist : dict
...
#
#     # Add random noise reference line λ = ln(3/2)
...
#
#     # Add chaos criterion annotation
...
def plot_degree_distribution(degree_dist, lambda_exp, r_squared, title, save_path):
    ...
    # Right panel requires positive values
    mask = p_values > 0
    if not np.any(mask):
    ...
    # ====== Fit both intercept A and lambda via log-domain linear regression ======
    ...
    # ====== Draw random reference line as exponential curve, aligned at median k ======
    lambda_random = np.log(3 / 2)

    k_ref = float(np.median(x))
    # Interpolate in log domain for numerical stability
    ...
    # Chaos criterion (using fitted lambda_fit for consistency)
...
def extract_hvg_features(G, time_series, window_size=50, stride=25):
    """
    Extract feature vectors from HVG

    Uses a sliding window approach to extract local HVG features:
    - Local degree distribution statistics
    ...
    for start in range(0, n - window_size + 1, stride):
        ...
        lambda_local, _, _ = fit_exponential_distribution(degree_dist)
...
def extract_hvg_node_features(G, time_series, k_neighbors=5):
    """
    Extract local HVG features for each node
...
class HVGAutoencoder(nn.Module):
    """
    Autoencoder for learning low-dimensional representations of HVG features

    Architecture:
    - Encoder: multi-layer MLP that progressively reduces to latent space
    - Decoder: symmetric MLP that reconstructs from the latent space
    """
...
class VariationalHVGAutoencoder(nn.Module):
    """
    Variational Autoencoder (VAE) for learning a smoother latent space
    """
...
def plot_latent_space(latent_vectors, labels=None, title='Latent Space', save_path='latent_space'):
    ...
    latent_vectors : np.ndarray
        latent space vectors (n_samples, latent_dim)
    labels : array-like
        sample labels (used for coloring)
    title : str
        plot title
    save_path : str
        save path
...
class SINDyLibrary:
    """
    SINDy candidate function library

    Generates polynomial and trigonometric candidate terms
    """
    ...
    def fit_transform(self, X):
        """
        Build the candidate function library

        Parameters:
        -----------
        X : np.ndarray
            state variable matrix (n_samples, n_features)

        Returns:
        --------
        Theta : np.ndarray
            candidate function library matrix
        """
...
class DiscreteSINDy:
    """
    Discrete-time SINDy algorithm

    Identifies discrete dynamics: x(t+1) = f(x(t))
    """
    ...
    def fit(self, X):
        """
        Fit the discrete SINDy model

        Parameters:
        -----------
        X : np.ndarray
            state sequence (n_samples, n_features)
        ...
        # Perform sparse regression for each output dimension
...
def plot_sindy_results(sindy_model, X, save_path):
    """
    Plot SINDy identification results
    """
...
def estimate_lyapunov_exponent(time_series, embedding_dim=3, time_delay=1,
    ...
    """
    Estimate the maximum Lyapunov exponent using the Wolf algorithm
    ...
    while current_idx < n_embedded - min_separation and count < max_iterations:
        # Find nearest neighbor (excluding temporally close points)
...
def estimate_lyapunov_rosenstein(time_series, embedding_dim=3, time_delay=1,
                                 mean_period=10):
    """
    Estimate the maximum Lyapunov exponent using the Rosenstein method
    ...
    for i in range(n_embedded - max_j):
        # Exclude temporally close points when finding nearest neighbor
...
def plot_poincare_section(time_series, embedding_dim=3, time_delay=1,
    ...
    section_value : float
        section value (default is median)
    title : str
        plot title
    save_path : str
        save path
    ...
    # Find section crossing points (forward crossings)
    crossings = []
    for i in range(n_embedded - 1):
        if embedded[i, section_dim] < section_value <= embedded[i + 1, section_dim]:
    ...
    ax1.grid(True, alpha=0.3)  # Return map (relationship between consecutive crossings)
    ax2 = axes[1]
    ...
    # Return map (relationship between consecutive crossings)
    ax2 = axes[1]
    if len(crossings) > 1 and len(other_dims) >= 1:
        ax2.scatter(crossings[:-1, other_dims[0]], crossings[1:, other_dims[0]],
                    s=20, alpha=0.6, c='red')
...
def plot_lyapunov_analysis(time_series, embedding_dim=3, time_delay=1,
    ...
    """
    Comprehensive plot for Lyapunov exponent analysis
    """
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))

    # Estimate Lyapunov exponent (Rosenstein method)
...
def comprehensive_analysis(data_dict, output_dir='./results'):
    ...
    results = {
        ...
        'lyapunov': {},
        'sindy_equations': {},
        'hvg_degree_dist': {},  # Store degree distribution data for Figure 3
        'lyapunov_divergence': {}  # Store Lyapunov divergence curve data for Figure 3
    }
    ...
    for idx, (province, data) in enumerate(processed_data.items()):
        ...
        for component in ['PC', 'Mobile']:
            ...
            lambda_exp, r_squared, p_value = fit_exponential_distribution(degree_dist)
            results['hvg_lambda'][ts_name] = {
                'lambda': lambda_exp,
                'r_squared': r_squared,
                'p_value': p_value
            }
            # Store degree distribution data for Figure 3
    ...
    # Apply SINDy to latent space trajectories for each province
    features_per_province = len(all_features) // len(province_list)

    # Store SINDy models and province latent data for Figure 4
    sindy_models = {}
    province_latents = {}
    ...
    for province, data in processed_data.items():
        ...
        for component in ['PC', 'Mobile']:
            ...
            # Additionally compute and store Rosenstein divergence curve for Figure 3
    ...
    # Calculate the number of features per province
    features_per_province = len(all_features) // len(province_list)
    ...
    return results, processed_data, sindy_models, province_latents, autoencoder
...
def plot_figure4_latent_sindy(results, processed_data, output_dir,
    ...
    # Create Figure 4: 2-row 3-column layout
    ...
    ax1.set_xlabel('Latent Dim 1', fontsize=16)
    ax1.set_ylabel('Latent Dim 2', fontsize=16)
    ax1.set_zlabel('Latent Dim 3', fontsize=16)
    ax1.set_title('(a) 3D Latent Space by Province', fontsize=18, fontweight='bold')
    ...
    for idx, province in enumerate(provinces):
        ...
        # Plot temporal trajectory of the first latent dimension
        t = np.arange(len(latent_province))
        ax2.plot(t, latent_province[:, 0], '-', linewidth=0.5,
                 color=province_colors[idx], label=f'{province} ($z_1$)', alpha=0.6)

    ax2.set_xlabel('Time Step', fontsize=18)
    ax2.set_ylabel('Latent Value (z1)', fontsize=18)
    ...
    # ax3.set_title('(c) AE/VAE Training Loss Curves', fontsize=18, fontweight='bold')
    # ax3.legend(loc='upper right', fontsize=14)
    # ax3.set_yscale('log')
    # ax3.grid(True, alpha=0.3)

    # ========== (c) SINDy prediction vs. actual latent values for Beijing ==========
    ax4 = fig.add_subplot(1, 4, 3)

    # Get Beijing latent data and SINDy model
    ...
    if beijing_province in sindy_models and beijing_province in province_latents:
        ...
        # Use the last 200 points as the test period
        ...
        # Plot three latent variable dimensions
        ...
        ax4.set_xlabel('Test Time Step', fontsize=18)
        ax4.set_ylabel('Latent Value', fontsize=18)
        ax4.set_title(f'(d) SINDy Prediction vs Actual ({beijing_province}, {test_len}-day)',
                      fontsize=14, fontweight='bold')
        ax4.legend(loc='upper right', fontsize=14, ncol=2)
        ax4.grid(True, alpha=0.3)
    else:
        ax4.text(0.5, 0.5, 'Beijing SINDy data not available',
                 ha='center', va='center', transform=ax4.transAxes, fontsize=18)
        ax4.set_title('(c) SINDy Prediction vs Actual', fontsize=18, fontweight='bold')

    # ========== (d) Coefficient magnitude heatmap across provinces and term categories ==========
    ax5 = fig.add_subplot(1, 4, 4)

    # Collect coefficients for all provinces
    if sindy_models:
        ...
        # Compute mean coefficient magnitude per category per province
        ...
        vmax = coef_matrix.max()
        vmin = coef_matrix.min()
        sns.heatmap(coef_matrix, annot=True, fmt='.3f',
                    cmap='YlOrRd',
                    vmin=vmin, vmax=vmax,
                    ax=ax5,
                    cbar_kws={'shrink': 0.8, 'label': 'Mean |Coefficient|'},
        ...
        ax5.set_yticklabels(ax5.get_yticklabels(), rotation=0,
                            fontsize=16)
        # ax5.set_xlabel('Term Category', fontsize=16)
        ax5.set_ylabel('Province', fontsize=16)
        ax5.set_title('(d) SINDy Coefficient Magnitude Heatmap',
                      fontsize=14, fontweight='bold')


    # # Hide the 6th subplot position (bottom-right of 2x3 grid)
    # ax6 = fig.add_subplot(2, 3, 6)
    # ax6.axis('off')

    # Add figure description text
    # summary_text = """
    # Figure 4 Summary:
    # ─────────────────────────────
...
def estimate_correlation_dimension(time_series, embedding_dim=3, time_delay=1, r_values=None):
    ...
    time_delay : int
        time delay
    r_values : array-like
        range of distance values for computing C(r)
    ...
    # Fit slope in log-log domain to get correlation dimension
    log_r = np.log(r_values)
    log_c = np.log(c_values)

    # Select linear region for fitting
...
def calculate_convex_hull_volume(time_series, embedding_dim=3, time_delay=1):
    """
    Compute the convex hull volume of the phase space attractor
    ...
    try:
        ...
        # If convex hull cannot be computed, return covariance matrix determinant as volume measure
...
def plot_figure5_phase_space(results, processed_data, output_dir):
    ...
    # Create Figure 5: complex layout
    ...
    # Upper portion: 3D phase portraits (2 rows x 4 columns)
    for p_idx, province in enumerate(provinces):
    ...
    # Compute section value (mean of z₁)
    ...
    # Get non-section dimensions - consistent with original plot_poincare_section
    section_dim = 0
    other_dims = [i for i in range(embedding_dim) if i != section_dim]

    # Return map (relationship between consecutive crossings) - identical to original code
    if len(crossings) > 1 and len(other_dims) >= 1:
    ...
    for province in provinces:
        for component in components:
            ...
            # Lyapunov exponent (retrieved from existing results)
...
def plot_figure3_chaos_characterization(results, processed_data, output_dir):
    ...
    ax1.text(ax1.get_xlim()[1] * 0.3, 0.12, f'λ_random = ln(3/2) ≈ {lambda_random:.3f}',
    ...
    ax3.legend(handles=legend_elements, loc='lower left', fontsize=16, ncol=2)
    ax3.set_xlabel('HVG λ (degree exponent)', fontsize=18)
    ax3.set_ylabel('Lyapunov $λ_{max}$ (Rosenstein)', fontsize=18)
    ax3.set_title('(c) HVG λ vs. Lyapunov $λ_{max}$', fontsize=18, fontweight='bold')
    ax3.grid(True, alpha=0.3)
    ...
    # Connect Lyapunov points between PC and Mobile
    ...
    ax4.legend(lines1 + lines2, labels1 + labels2, loc='lower left', fontsize=14)
...
def generate_table2_annual_trend(data_dict, output_dir):
    ...
    # Compute annual mean total index (PC + Mobile) per province
    ...
    # Compute rate of change (from peak year to 2024)
    delta_row = {'Year': 'Δ(%)'}
    for province in provinces_order:
...
def generate_table3_chaos_analysis(results, output_dir):
    ...
    Columns: Province, Channel, HVG λ, R^2, λ_max(Rosenstein), λ_max(Wolf), Chaos Strength
    """
    rows = []
    provinces_order = ['Beijing', 'Guangdong', 'Henan', 'Shanghai']
    ...
    for province in provinces_order:
        for channel in ['PC', 'Mobile']:
            ...
            p_value = results['hvg_lambda'][ts_name]['p_value']
            lyap_rosenstein = results['lyapunov'][ts_name]['rosenstein']
            lyap_wolf = results['lyapunov'][ts_name]['wolf']

            # Determine chaos strength
            # Based on combined assessment of HVG λ and Lyapunov exponent
...
def compute_autoencoder_metrics(autoencoder, vae, X_tensor, all_labels, provinces, features_per_province, device):
    """
    Compute reconstruction performance metrics for the autoencoder
    """
    autoencoder.eval()
    vae.eval()
    ...
    with torch.no_grad():
        ...
        for idx, province in enumerate(provinces):
            ...
            # Compute correlation coefficients (after flattening)
...
def compute_sindy_metrics(sindy_models, province_latents, provinces):
    """
    Compute statistical metrics of SINDy coefficients
    """
    sindy_results = []
    ...
    for province in provinces:
        ...
        for i, name in enumerate(feature_names):
            if name == '1':
                ...
                quadratic_indices.append(i)  # interaction terms also counted as quadratic
            elif 'sin' in name or 'cos' in name:
        ...
        # Compute number of active terms
        active_terms = np.sum(np.abs(coefs) > 1e-10)
        total_terms = coefs.size

        # Compute NRMSE - one-step prediction error at discrete time points
        # x(t) -> x(t+1) prediction
        X_in = latent[:-1]  # x(t), shape: (T-1, n_dims)
        X_out_actual = latent[1:]  # x(t+1) actual values
        X_out_pred = model.predict(X_in)  # x(t+1) predicted values

        # Compute NRMSE per dimension, then average
        ...
        for d in range(n_dims):
            ...
            # Normalize: use the range of actual values (max - min)
        ...
        # Take the mean NRMSE across all dimensions
...
def save_results(results, output_dir):
    ...
    def convert_numpy(obj):
        if isinstance(obj, np.ndarray):
            ...
            return {(int(k) if isinstance(k, (np.int64, np.int32)) else k): convert_numpy(v) for k, v in obj.items()}
